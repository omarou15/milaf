...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <button onClick={() => setOpen(true)} className="lg:hidden fixed top-4 left-4 z-30 w-9 h-9 bg-[#0d0f18] border border-[#181c2c] rounded-lg flex items-center justify-center text-white text-lg">☰</button>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-60 bg-[#0d0f18] border-r border-[#181c2c] flex flex-col transform transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <div className="p-5 border-b border-[#181c2c] flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[#3a3f5c] ml-1.5 text-xs">ملف</span>
            </div>
          </div>
          <button onClick={() => setOpen(false)} className="lg:hidden text-[#6b7290] hover:text-white">✕</button>
        </div>
        ...
        {/* Credits widget */}
        {credits && plan && (
          <Link href="/settings" className="mx-3 mb-2 p-3 bg-[#0a0b12] border border-[#1e2235] rounded-xl hover:border-[#2a2f48] transition-colors block">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[10px] font-semibold text-[#6b7290] uppercase tracking-wider">Crédits</span>
            ...
            <div className="flex items-baseline gap-1 mb-1.5">
              ...
              <span className="text-xs text-[#6b7290]">/ {credits.total}</span>
            </div>
