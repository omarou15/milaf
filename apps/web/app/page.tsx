...
export default function LandingPage() {
  return (
    <main className="min-h-[100dvh] bg-page text-white overflow-x-hidden grain">
      ...
      <nav className="flex items-center justify-between px-6 py-5 max-w-6xl mx-auto fade-up">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-muted text-sm ml-0.5">ملف</span>
        </div>
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <Link href="/login" className="text-sm text-secondary hover:text-white transition-colors px-3 py-1.5 hidden sm:block">Connexion</Link>
          <Link href="/signup" className="text-sm font-semibold px-5 py-2.5 bg-[var(--text-primary)] text-[var(--bg)] hover:bg-gray-100 rounded-full transition-colors">
      ...
      <section className="px-6 pt-20 pb-24 text-center max-w-4xl mx-auto relative">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-surface border border-theme rounded-full text-xs text-secondary font-medium mb-8 fade-up delay-1">
          ...
          <p className="text-secondary text-lg sm:text-xl max-w-xl mx-auto mb-10 leading-relaxed fade-up delay-3" style={{ fontFamily: 'var(--font-body)' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center fade-up delay-4">
            <Link href="/signup"
              className="px-8 py-4 bg-[var(--text-primary)] text-[var(--bg)] font-bold rounded-full transition-all hover:shadow-lg hover:shadow-white/10 text-sm">
              Commencer gratuitement →
            </Link>
            <Link href="/login"
              className="px-8 py-4 bg-surface border border-theme hover:border-theme text-white font-semibold rounded-full transition-all text-sm">
              Se connecter
            </Link>
          </div>
          <p className="text-xs text-muted mt-5 fade-up delay-5">Pas de carte bancaire · 10 crédits gratuits</p>
        </div>
      </section>

      {/* ── Demo visual ── */}
      <section className="px-6 pb-24 max-w-4xl mx-auto fade-up delay-6">
        <div className="bg-surface border border-theme rounded-2xl p-6 sm:p-8 glow-indigo">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-2.5 h-2.5 rounded-full bg-red-500/60" />
            <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/60" />
            <div className="w-2.5 h-2.5 rounded-full bg-green-500/60" />
            <span className="text-[10px] text-muted ml-2 font-mono">milaf.vercel.app/chat</span>
          </div>
              ...
              <div className="space-y-2 max-w-[85%]">
                ...
                <div className="border border-emerald-500/30 bg-emerald-500/[0.03] rounded-xl px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-primary">Facture Dupont — Mars 2026</div>
                    <div className="text-[10px] text-secondary">Document complet avec TVA, mentions légales</div>
                  </div>
      ...
      <section className="px-6 pb-24 max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-secondary">Du simple Word balisé au clonage IA de n&apos;importe quel document.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {TIERS.map((t, i) => (
            <div key={t.tier}
              className={`p-6 bg-surface border border-theme rounded-2xl relative overflow-hidden group hover:border-theme transition-all hover-lift fade-up delay-${i + 1}`}>
              ...
              <h3 className="text-lg font-bold text-primary mb-2" style={{ fontFamily: 'var(--font-display)' }}>{t.title}</h3>
              <p className="text-sm text-secondary leading-relaxed mb-4">{t.desc}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={s.n} className={`flex items-center gap-4 p-3 bg-page/60 border border-amber-500/10 rounded-xl slide-in delay-${i + 1}`}>
      ...
      <section className="px-6 pb-24 max-w-6xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-secondary">16 templates prêts à l&apos;emploi. Des centaines à venir.</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {SECTORS.map((s, i) => (
            <div key={s.label} className={`p-4 bg-surface border border-theme rounded-2xl text-center hover:border-theme transition-all hover-lift fade-up delay-${i + 1}`}>
              <div className="text-2xl mb-2">{s.emoji}</div>
              <div className="text-xs font-semibold text-primary">{s.label}</div>
              <div className="text-[10px] text-secondary mt-0.5 leading-tight">{s.sub}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 pb-24 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={c.vs} className="p-6 bg-surface border border-theme rounded-2xl hover-lift">
      ...
      <section className="px-6 pb-24 max-w-5xl mx-auto" id="pricing">
        <div className="text-center mb-12">
          ...
          <p className="text-secondary">Commencez gratuitement. Pas de carte bancaire.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {PLANS.map((p) => (
            <div key={p.name}
              className={`p-6 rounded-2xl relative hover-lift ${p.popular ? "border-2 bg-surface" : "border border-theme bg-surface"}`}
              ...
              <div className="flex items-baseline gap-0.5 mb-0.5">
                ...
                {Number(p.price) > 0 && <span className="text-sm text-secondary">/mois</span>}
              </div>
              <div className="text-xs text-secondary mb-5">{p.credits} crédits/mois</div>
              <ul className="space-y-2 mb-6">
                {p.features.map((f) => (
                  <li key={f} className="text-xs text-secondary flex items-start gap-1.5">
      ...
      <section className="px-6 pb-24 max-w-3xl mx-auto text-center">
        <div className="bg-gradient-to-br from-indigo-600/10 to-cyan-600/5 border border-indigo-500/20 rounded-3xl p-10 glow-indigo">
          ...
          <p className="text-secondary mb-8">10 crédits gratuits. Aucune carte requise. Prêt en 30 secondes.</p>
          <Link href="/signup"
            className="inline-flex items-center gap-2 px-8 py-4 bg-[var(--text-primary)] text-[var(--bg)] font-bold rounded-full transition-all hover:shadow-lg hover:shadow-white/10 text-sm">
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-theme px-6 py-8 max-w-6xl mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm text-secondary">Mi-Laf ملف</span>
            <span className="text-xs text-muted">· Génération documentaire par IA</span>
          </div>
